...
class Dataset(object):
	...
	def featurize(self, kbts_by_rel, featurizers, vectorizer=None, vectorize=True):
		...
		"""
		glove_featurizer=featurizers[-1]
		featurizers=featurizers[:-1]
		...
		vector_counter_by_rel=defaultdict(list)
		for rel, kbts in kbts_by_rel.items():
			for kbt in kbts:
				combined_glove_vector = glove_featurizer(kbt, self.corpus)
				vector_counter_by_rel[rel].append(combined_glove_vector.reshape(1,-1))

		for rel in vector_counter_by_rel.keys():
			vector_counter_by_rel[rel]=np.concatenate(vector_counter_by_rel[rel],axis=0)


		for rel in vector_counter_by_rel.keys():
			assert (vector_counter_by_rel[rel].shape[0]==feat_matrices_by_rel[rel].shape[0])
		feat_matrices_by_rel[rel]=hstack((sparse.csr_matrix(vector_counter_by_rel[rel]),feat_matrices_by_rel[rel]))
		print("Hurray completed all the assertions")
...
def train_models(
	...
	models = {}
	scalers={}
	for rel in tqdm(splits['all'].kb.all_relations):
		models[rel] = model_factory()
		if scaling:
			scaler = StandardScaler(with_mean=False)
			scaler.fit(train_X[rel])
			models[rel].fit(scaler.transform(train_X[rel]), train_y[rel])
			scalers[rel]=scaler
		else:
	...
	return {
		...
		'vectorize': vectorize,'scalers':scalers}


def predict(splits, train_result, split_name='dev', sampling_rate=0.1, vectorize=True,scaling=False):
	...
	test_X, _ = assess_dataset.featurize(
		...
		vectorize=vectorize)
	predictions = {}
	scalers=train_result['scalers']
	for rel in train_result['all_relations']:
		if scaling:
			scaler=scalers[rel]
			predictions[rel] = train_result['models'][rel].predict(scaler.transform(test_X[rel]))
		else:
...
def experiment(
	...
	evaluate_predictions(
		...
		test_y,
		verbose)
	return train_result,predictions,test_y


def examine_model_weights(train_result, k=20, verbose=True):
	vectorizer = train_result['vectorizer']
